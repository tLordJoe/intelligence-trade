...
### Exact total-reconciliation investigations

Valid filings now retain both declared and computed row/value totals. A mismatch throws a structured `TableReconciliationError` with the accession, source hash, period, value units, declared/computed totals and differences. Archive replay preserves those diagnostics under `reconciliationIssues` while keeping the filing rejected and affected manager blocked. There is no monetary tolerance or automatic override, even for a one-dollar mismatch.

- **FMR `0000315066-26-002260`, June 30, 2026:** declared and computed rows both 13,978. Declared value USD 2,303,878,429,728; independent sum of every source row USD 2,303,878,429,723, a negative USD 5 difference. Both XML traversal and a separate lexical row/value tally agree. The source contains one complete information table and the normal X0202 dollar format. This is a source-level total discrepancy, not evidence of dropped rows or a thousand-dollar conversion error. Rounding is a possible explanation but is not established by the archived submission; the mismatch is **not waived**.
- **JPMorgan `0000019617-26-000212`, March 31, 2026:** cover declares 33,063 rows and USD 1,557,139,501,048; its single source information table contains only 378 rows totaling USD 3,085,467,664. Namespace-aware traversal and separate prefixed-tag counts agree, so the parser did not skip the missing 32,685 rows. The archived complete submission closes normally. Same-day `13F-HR/A` accession `0000019617-26-000225` explicitly says `RESTATEMENT`; it contains 33,063 rows and exactly USD 1,557,139,501,048. The evidence supports an incomplete original table and a reconciled restatement, **not automatic replacement**. The family remains held until amendment adjudication is implemented and reviewed.

All six collected managers were replayed after this change: Berkshire, State Street and Goldman Sachs have eligible latest-quarter pairs; Vanguard, FMR and JPMorgan remain held. Goldman's older amended family remains held even though its latest pair is usable. Seven named cohort candidates remain uncollected. The replay is not publication approval.
