...
export default async function SectorPage({ params }: { params: Promise<{ sector: string }> }) {
  ...
  return <><Navbar showCompare={false} /><main className="home-shell sector-page">
    ...
    <section id="companies" className="sector-page-section" aria-labelledby="sector-companies-title">
      <div className="home-section-intro"><h2 id="sector-companies-title">Companies in this sector</h2><p>A starting list, not a ranking. Open a company to inspect its House disclosures.</p></div>
      <div className="sector-company-grid">{active.tickers.map(ticker => {
        const activity = window.companies.find(c => c.ticker === ticker);
        return <Link key={ticker} href={`/congress?ticker=${encodeURIComponent(ticker)}`} className="sector-company-card">
          <span className="home-ticker-tile" aria-hidden="true">{ticker}</span><div><h3>{ticker}</h3>{activity && <p>{activity.name}</p>}
          <small>{activity ? `${activity.buyers} distinct buying ${activity.buyers === 1 ? "filer" : "filers"} · ${activity.purchases} purchase ${activity.purchases === 1 ? "record" : "records"}` : "No eligible YTD House purchases in this view"}</small><span className="home-text-link">View disclosures →</span></div>
        </Link>;
      })}</div>
    ...
    <section id="funds" className="sector-page-section" aria-labelledby="sector-funds-title">
      ...
      <p className="home-muted">Names and sector associations checked against issuer sources on {SECTOR_FUND_CHECKED}. These funds may cover different companies and use different weighting rules. Current holdings, fees, performance and overlap are not yet connected here.</p>
    </section>
