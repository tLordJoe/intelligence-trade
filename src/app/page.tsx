...
export default function Home() {
  ...
  const insiders = readApprovedInsiderRelease(insiderRelease);
  const houseWindows = {
  ...
  const insiderWindows = insiders ? {
    ytd: buildInsiderHomeWindow(insiders, "ytd", asOf),
    "30": buildInsiderHomeWindow(insiders, "30", asOf),
    "90": buildInsiderHomeWindow(insiders, "90", asOf),
  } : null;
  const windows = Object.fromEntries((["ytd", "30", "90"] as HomePeriod[]).map(period => {
    const congress = senateWindows ? mergeHomeWindows(houseWindows[period], senateWindows[period]) : houseWindows[period];
    return [period, insiderWindows ? mergeHomeWindows(congress, insiderWindows[period]) : congress];
  })) as typeof houseWindows;
  return <>
