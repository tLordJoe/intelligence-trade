...
export function sourceFailure(error: unknown): string {
  ...
  return `${error.name}: ${error.message}${cause ? `; cause=${String(cause.code ?? cause.message ?? "unknown")}` : ""}`;
}
...
export class SecReader {
  ...
  constructor(userAgent = process.env.SEC_USER_AGENT) {
    if (!userAgent?.trim() || /[\r\n]/.test(userAgent) || !/[^\s@]+@[^\s@]+\.[^\s@]+/.test(userAgent)) throw new Error("SEC_USER_AGENT must identify Outfox and include its monitored contact email");
  ...
  async read(raw: string): Promise<Buffer> {
    ...
    this.last = Date.now();
    const response = await fetch(url, { redirect: "error", signal: AbortSignal.timeout(45000), headers: { "User-Agent": this.userAgent, Accept: "application/json,text/plain" } });
