...
export function reconcileInstitutions(filings: InstitutionalFiling[], blockedManagers: string[] = []) {
  ...
  for (const [family, entries] of families) {
    const reason = entries.some(f => blockedManagers.includes(f.reference.cik)) ? "manager_has_unparsed_or_failed_filings" :
      ...
      entries.some(f => f.confidentialOmitted || f.reportType !== "13F HOLDINGS REPORT" || f.otherIncludedManagers > 0) ? "confidential_or_combination_report_requires_review" : null;
...
const holdingKey = (row: Holding) => JSON.stringify([row.cusip, row.titleOfClass, row.quantityType, row.putCall, row.investmentDiscretion, row.otherManagers, row.figi]);
