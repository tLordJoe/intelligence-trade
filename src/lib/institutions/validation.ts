import type { InstitutionalFiling, InstitutionalNotice,TableReconciliationIssue } from "./parse.ts";
...
export function validateManagerCorpus(filings: InstitutionalFiling[], failures: string[] = [], notices:InstitutionalNotice[] = [],reconciliationIssues:TableReconciliationIssue[] = []) {
  ...
  const latestPeriods = [...new Set([...filings,...notices,...reconciliationIssues].map(f => f.period))].sort().reverse().slice(0, 2);
  ...
  return { ciks, notices,reconciliationIssues, latestReportedPeriods: latestPeriods, parsedFilings: filings.length,
    parsedRows: filings.reduce((sum,f) => sum + f.holdings.length,0),
    filings: filings.map(f => ({ accession:f.reference.accession, filedDate:f.reference.filedDate, period:f.period, manager:f.managerName, rows:f.holdings.length, declaredEntries:f.declaredEntries,computedEntries:f.computedEntries,declaredValue:f.declaredValue,computedValue:f.computedValue,valueUnit:f.valueUnit,sourceSha256:f.sourceSha256, sourceUrl:f.sourceUrl, amendment:f.amendment, amendmentType:f.amendmentType })),
