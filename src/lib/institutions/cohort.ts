...
export function assessManagerRun(directory:string, manager:ManagerCandidate, plan:CohortPlan) {
  const {run,filings,notices,failures,reconciliationIssues}=replayInstitutionRun(directory);
  ...
  const result=validateManagerCorpus(filings,failures,notices,reconciliationIssues),comparison=result.comparison;
  ...
  return {cik:manager.cik,name:manager.name,secName:source.name,runId:run.runId,identityMatched,status:ready?"ready_for_review":"held",
    ...
    filings:result.filings,notices,reconciliationIssues,held:result.held,failures,latestReportedPeriods:result.latestReportedPeriods,
