...
import { SecReader, references, allowedSecUrl } from "../src/lib/institutions/source.ts";
...
try {
  ...
  for(const reference of run.references){
    ...
    catch(error){run.filings.push({reference,failure:String(error)});}
  }
  run.complete=true; // Enumeration complete; individual failures still hold manager during replay.
}catch(error){run.failure=String(error);process.exitCode=1;}
