...
const companies = catalog.entries.filter((e) => e.kind === "company");
const funds = catalog.entries.filter((e) => e.kind === "fund");
const sponsors = catalog.entries.filter((e) => e.kind === "sponsor");
...
const companyTickers = companies.flatMap(tickersOf);
const resolvedTickers = companies.filter((e) => e.status === "resolved").flatMap(tickersOf);
...
const report = {
  ...
  companies: { ...tally(companies), issuersInUniverse: catalog.universes.companies?.identities ?? companies.length, issuersResolved: companies.filter((e) => e.status === "resolved").length,
...
for (const [name, u] of Object.entries(catalog.universes)) lines.push(`- **${name}** universe: ${u.identities} identities${u.tickers ? ` (${u.tickers} tickers)` : ""}, as of ${u.asOf}. Source: ${u.source}.${u.note ? ` ${u.note}` : ""}`);
